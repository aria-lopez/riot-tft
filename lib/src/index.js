...
class RiotTFT {
    ...
    updatePayload(items) { this.payload = {...this.payload, ...items}; }
    ...
    async getMatchByMatchId(matchId = null) {
        try {
            if (!this.payload?.matchId && matchId === null) { throw new Error('getMatchByMatchId requires either a matchId in the payload or a matchId as an argument'); };
            const selectedMatchId = matchId ? matchId : this.payload.matchId;
            const key = `matchByMatchId-${selectedMatchId}`;
